...
os.environ[
    'RADICAL_PILOT_DBURL'] = "mongodb://smush:key1209@ds233551.mlab.com:33551/db_repex_2"
...
if __name__ == '__main__':
    ...
    appman = AppManager(
        autoterminate=False, port=33215)  # Create Application Manager
    appman.resource_desc = res_dict  # Assign resource manager to the Application Manager

    Exchange = synchronousExchange.InitCycle(
        Replicas, Replica_Cores, MD_Executable, ExchangeMethod, timesteps)
    ...
    pwd = os.getcwd()
    session = ra.Session(
        sid='./%s' % appman.sid, stype='radical.entk', src=pwd)
    ...
    md_scheduling_durations = md.duration(
        [re.states.SCHEDULED, re.states.SUBMITTED])
    ...
    ex_scheduling_durations = ex.duration(
        [re.states.SCHEDULED, re.states.SUBMITTED])
